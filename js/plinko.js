class Plinko{
    constructor(width, height, ctx, credit, ballCostInput, rowsInput, riskInput, sound, autoplayInput){
        this.width = width;
        this.height = height;
    ...
    drawCircle(ball){
        ...
        this.ctx.beginPath();
        this.ctx.ellipse(ball.position.x, ball.position.y, ball.r, ball.r, 0, 0, 2 * Math.PI);
        this.ctx.fill();
    }
    ...
    drawRectangle(rect){
        ...
        this.ctx.beginPath();
        this.ctx.roundRect(rect.position.x, rect.position.y, rect.width, rect.height, 5);
        this.ctx.fill();
    }
    ...
    drawBasket(basket){
        ...
        ctx.font = "10px Arial";
        ctx.fillText(basket.multiplier, basket.position.x + basket.width / 2 - 5, basket.position.y + basket.height / 2);
    }
    ...
    drawRecord(record){
        ...
        ctx.font = "10px Arial";
        ctx.fillText('BET: ' + record.ballCost + '   PAYOUT: ' + record.basketMultiplier + '   PROFIT: ' + parseFloat(record.ballCost * record.basketMultiplier - record.ballCost).toFixed(2), record.position.x + 5, record.position.y + record.height / 2);
    }
    ...
    spawnBall(){
        ...
        let newBall = new Ball(this.ballSize, new Vector(this.width / 2, this.ballSize), parseFloat(this.ballCostInput.value), 'red');
    ...
    setPegs(){
        ...
        for(let i=0; i < this.rows; i++){
            ...
            for( let j=0; j < startNumOfPegs; j++){
                let peg = new Peg(this.ballSize / 2, new Vector(this.width / 2 + padding + shift * i, i * 40 + 40), 1, 'white');
                this.pegs.push(peg)
                padding += 40;
            }
            startNumOfPegs++;
    ...
    setBaskets(){
        ...
        for(let i=0; i <this.rows+1; i++){
            ...
            let basket = new Basket(30, 30, new Vector(this.width / 2 - 15 + padding + shift, (this.rows-1) * 40 + 40), multipliers[i]);
    ...
    ballOutsideGame(ball, basket, left){
        if(left){
            if(ball.position.x < basket.position.x && ball.position.y >= basket.position.y 
    ...
    ballInBasket(ball){
        for(let basket of this.baskets){
            if(this.baskets.indexOf(basket) == 0 || this.baskets.indexOf(basket) == this.baskets.length - 1){
            ...
            if(ball.position.x >= basket.position.x && ball.position.x <= basket.position.x + basket.width
                && ball.position.y >= basket.position.y && ball.position.y <= basket.position.y + basket.height){
    ...
    adjustBall(ball, peg){
        ...
        if(distanceX > 0){
            ball.position.x -= ball.r + peg.r - Math.sqrt(Math.abs(distanceX) ** 2 + Math.abs(distanceY) ** 2);
        }
        ...
        if(distanceY > 0){
            ball.position.y -= ball.r + peg.r - Math.sqrt(Math.abs(distanceX) ** 2 + Math.abs(distanceY) ** 2);
        }
    ...
    updateBallPosition(ball){
        ...
        for(let peg of this.pegs){
            if(Math.sqrt(Math.abs(peg.position.x - ball.position.x) ** 2 + Math.abs(peg.position.y - ball.position.y) ** 2) <= ball.r + peg.r){
                ...
                if(ball.initialRandomness != 0){
                    ball.addForce(new Vector(ball.initialRandomness * this.randomPositiveOrNegativeOne(), (ball.position.y - peg.position.y) / 4));
                    ball.initialRandomness = 0;
                }
                else{
                    let impactVector = new Vector((ball.position.x - peg.position.x), (ball.position.y - peg.position.y));
                    let cSideTriangle = Math.sqrt(impactVector.x ** 2 + impactVector.y ** 2);
                    ...
                    let angleShift = Math.asin(1 / cSideTriangle * divider) * (180/Math.PI);
    ...
    draw(){
        ...
        for(let i=this.history.history.length - 1; i >= 0 && i > this.history.history.length - (1 + this.history.maxVisibleRecords); i--){
